...
final class Version20201226203000 extends AbstractMigration
{
    ...
    {
        return 'Added \'logsession_latest\' for listeners and signal type count columns for log_sessions';
    }
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('ALTER TABLE `rxx`.`log_sessions`  ADD COLUMN `logs_DGPS` INT(10) UNSIGNED DEFAULT NULL AFTER `logs`');
        $this->addSql('ALTER TABLE `rxx`.`log_sessions`  ADD COLUMN `logs_DSC` INT(10) UNSIGNED DEFAULT NULL AFTER `logs_DGPS`');
        $this->addSql('ALTER TABLE `rxx`.`log_sessions`  ADD COLUMN `logs_HAMBCN` INT(10) UNSIGNED DEFAULT NULL AFTER `logs_DSC`');
        $this->addSql('ALTER TABLE `rxx`.`log_sessions`  ADD COLUMN `logs_NAVTEX` INT(10) UNSIGNED DEFAULT NULL AFTER `logs_HAMBCN`');
        $this->addSql('ALTER TABLE `rxx`.`log_sessions`  ADD COLUMN `logs_NDB` INT(10) UNSIGNED DEFAULT NULL AFTER `logs_NAVTEX`');
        $this->addSql('ALTER TABLE `rxx`.`log_sessions`  ADD COLUMN `logs_OTHER` INT(10) UNSIGNED DEFAULT NULL AFTER `logs_NDB`');
        $this->addSql('ALTER TABLE `rxx`.`log_sessions`  ADD COLUMN `logs_TIME` INT(10) UNSIGNED DEFAULT NULL AFTER `logs_OTHER`');
    }
